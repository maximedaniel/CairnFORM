...
class CairnFORM:
    ...
    def __init__(self):
        ...
        self.mqtt.connect(ipAddress, 1883, 60)
    ...
    def input(self, client, userdata, msg):
        try:
            ...
            for instruction in instructions:
                    ...
                    if not (0 <= target[0] <= 255 and 0 <= target[1] <= 255  and 0 <= target[2] <= 255 and 0 <= target[3] <= 255):
                        raise Exception("assertion failed for (0 <= red <= 255 and 0 <= green <= 255  and 0 <= blue <= 255 and 0 <= position <= 255)")
